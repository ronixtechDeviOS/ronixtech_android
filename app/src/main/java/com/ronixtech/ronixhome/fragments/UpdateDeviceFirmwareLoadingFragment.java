...
public class UpdateDeviceFirmwareLoadingFragment extends androidx.fragment.app.Fragment {
    ...
    private void getDeviceConfig(Device device){
        ...
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                //Do something after 50000ms
                DeviceConfigGetter deviceChecker = new DeviceConfigGetter(UpdateDeviceFirmwareLoadingFragment.this, device);
    ...
    public void goToSuccessFragment(){
        if(MainActivity.getInstance() != null && MainActivity.isResumed){
            if(getFragmentManager() != null){
                ...
                successFragment.setSuccessSource(Constants.SUCCESS_SOURCE_DEVICE_FIRMWARE);
    ...
    public class DeviceConfigGetter extends AsyncTask<Void, Void, Void> {
        ...
        public DeviceConfigGetter(UpdateDeviceFirmwareLoadingFragment addDeviceConfigurationPreparingFragment, Device device) {
            this.device = device;
                        ...
                        if(jsonObject != null){
                            ...
                            if(unitStatus != null && unitStatus.has("U_W_STT")){
                                ...
                                if(wifiStatus != null) {
                                    ...
                                    if(wifiStatus.has("U_W_TYP")){
                                        String wifiTypeString = wifiStatus.getString("U_W_TYP");
                                        if(wifiTypeString != null && wifiTypeString.length() >= 1){
                                            int wifiType = Integer.parseInt(wifiTypeString);
                                            device.setDeviceTypeID(wifiType);
                                        }
                                    }
